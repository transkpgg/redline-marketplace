    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-12">
        <input 
          ...
          className="bg-black/50 border-2 border-white/20 p-4 text-white focus:border-[var(--accent-red)] outline-none flex-grow font-[var(--font-anton)] uppercase tracking-widest text-xs md:text-sm transition-colors"
          value={search}
          ...
          {filteredProducts.map(product => (
            <div key={product.id} className="group cursor-pointer border-r border-b border-white/10 p-6 hover:bg-white/5 transition-colors" onClick={() => setSelectedProduct(product)}>
              <div className="product-image-wrapper aspect-[4/5] relative mb-6 overflow-hidden border border-white/10 bg-black">
                ...
                <div className="absolute top-4 right-4 bg-[var(--accent-red)] text-white text-xs font-bold px-3 py-1 tracking-wider font-[var(--font-anton)] z-10">
                  BARU
                </div>
              </div>
              <div className="flex justify-between items-center pb-4 border-b border-white/10 mb-4">
                <h3 className="font-[var(--font-anton)] font-bold tracking-widest text-base group-hover:text-[var(--accent-red)] transition-colors">{product.name}</h3>
                <p className="text-[var(--text-secondary)] font-[var(--font-anton)]">${product.price.toFixed(2)}</p>
              </div>
              <button className="w-full py-4 bg-transparent border-2 border-white/20 text-white font-black font-[var(--font-anton)] text-xs tracking-[0.2em] uppercase group-hover:bg-[var(--accent-red)] group-hover:text-white group-hover:border-[var(--accent-red)] group-hover:shadow-[0_0_30px_rgba(255,0,60,0.4)] transition-all duration-300 flex items-center justify-center gap-2">
