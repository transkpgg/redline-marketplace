    ...
    <header className="fixed w-full top-0 z-[100]">
      ...
      <nav className="flex justify-between items-center px-6 md:px-16 py-4 backdrop-blur-xl border-b border-white/10 bg-black/80">
        ...
        <div className="flex items-center gap-6 z-50">
          ...
          {/* Mobile Menu Toggle */}
          <button className="md:hidden text-white hover:text-[var(--accent-red)] transition-colors" onClick={() => setIsOpen(!isOpen)}>
            {isOpen ? <X size={28} /> : <Menu size={28} />}
          </button>
        </div>

        {/* Mobile Menu Backdrop */}
        <div className={`md:hidden fixed inset-0 bg-black/80 backdrop-blur-sm z-40 transition-opacity duration-300 ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`} onClick={() => setIsOpen(false)}></div>

        {/* Mobile Menu Sidebar (Menyamping) */}
        <div className={`md:hidden fixed top-0 right-0 h-full w-[70%] max-w-[280px] bg-[#050505] border-l border-white/10 z-50 flex flex-col justify-center pl-8 shadow-2xl transition-transform duration-500 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <button className="absolute top-6 right-6 text-white/50 hover:text-[var(--accent-red)] transition-colors" onClick={() => setIsOpen(false)}>
            <X size={32} />
          </button>
          ...
          <div className="flex flex-col gap-8 w-full pr-8">
            ...
            <div className={`mt-8 flex flex-col gap-4 transition-all duration-500 delay-400 ${isOpen ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-0'}`}>
              <Link href="#shop" onClick={() => setIsOpen(false)} className="text-white/50 hover:text-[var(--accent-red)] uppercase tracking-[0.3em] text-xs font-bold transition-colors w-max">PAKAIAN</Link>
              <Link href="#shop" onClick={() => setIsOpen(false)} className="text-white/50 hover:text-[var(--accent-red)] uppercase tracking-[0.3em] text-xs font-bold transition-colors w-max">AKSESORIS</Link>
            </div>
          </div>
        </div>
      </nav>
